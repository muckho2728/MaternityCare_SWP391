...
namespace MaternityCare_Backend.API.Controllers
{
	...
	public class DoctorsController : ControllerBase
	{
		...
		{
			var pagedResult = await serviceManager.DoctorService.GetDoctors(doctorParameters, ct);
		...
		[Authorize]
		public async Task<IActionResult> GetActiveDoctors([FromQuery] DoctorParameters doctorParameters, CancellationToken ct)
		{
			var pagedResult = await serviceManager.DoctorService.GetActiveDoctors(doctorParameters, ct);
			Response.Headers.Append("X-Pagination", JsonSerializer.Serialize(pagedResult.metaData));
			return Ok(pagedResult.doctors);
		}
