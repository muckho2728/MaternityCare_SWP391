...
namespace MaternityCare_Backend.Service.UserServices
{
	public interface IUserService
	{
		...
		Task<UserForReturnDto> GetUserById(Guid userId, bool trackChange, CancellationToken ct = default);
		Task<UserForReturnDto> GetUserByToken(string jwt, bool trackChange, CancellationToken ct = default);
		Task<(IEnumerable<UserForReturnDto> users, MetaData metaData)> GetUsers(UserParameters userParameters, bool trackChange, CancellationToken ct = default);
		Task ChangeActiveStatus(Guid userId, CancellationToken ct = default);
		Task UpdateUser(Guid userId, UserForUpdateDto userForUpdateDto, bool trackChange, CancellationToken ct = default);
